...
class MigrationRunner {
  ...
  async migrationRunner(): Promise<number> {
    ...
    fs.readdirSync(migrationDir).map((file) => {
      ...
      up.map((diffContext: DiffData) =>
        queries.push({ rawQuery: diffContext.query, filename: file })
      );
    });

    const filenames = await Promise.all(
      queries.map(async ({ rawQuery, filename }) => {
        try {
          const alreadyRunMigation = await query(
            `SELECT name FROM \`${this.databaseName}\`.\`${this.metaTableName}\` where name = '${filename}' LIMIT 1;`
          );
